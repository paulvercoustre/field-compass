...
class GenerateRuleRequest(BaseModel):
    ...
    prompt: str = Field(
        ..., min_length=1, max_length=1000, description="Natural language rule description"
    )
...
async def generate_rule_from_natural_language(
    ...
    try:
        ...
        logger.info(
            f"Successfully generated rule for survey {payload.survey_id}: {rule_data.get('description')}"
        )
...
async def suggest_validation_rules(
    ...
    try:
        ...
        logger.info(
            f"Successfully generated {len(suggestions)} rule suggestions for survey {payload.survey_id}"
        )
...
def _extract_variables_from_config(survey_config: SurveyConfig) -> list[dict[str, Any]]:
    ...
    for question in survey_sheet:
        ...
        q_label = question.get(
            label_col_survey, question.get("label::English (en)", question.get("label", ""))
        )

        # Skip metadata fields and groups (but keep 'calculate' as it can contain numeric calculations)
        if not q_name or q_type in [
            "begin_group",
            "end_group",
            "begin_repeat",
            "end_repeat",
        ...
        var_info = {"name": q_name, "type": q_type, "label": q_label}
