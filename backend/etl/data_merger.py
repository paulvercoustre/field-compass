...
def is_edited_submission(kobo_data: dict[str, Any] | None = None) -> tuple[bool, str]:
    """
...
def parse_kobo_submission(kobo_data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        if (
            "Z" in submission_time_str
            or "+" in submission_time_str
            or submission_time_str.count("-") > 2
        ):
            # Has timezone info
    ...
    submission_data = dict(kobo_data.items())
        ...
        if attachments:
            for attachment in attachments:
                if isinstance(attachment, dict):
                    filename = attachment.get("filename", "") or attachment.get(
                        "media_file_basename", ""
                    )
...
def merge_submission(
    ...
    kobo_data: dict[str, Any] | None = None,
) -> tuple[SubmissionCurrent, SubmissionHistory | None, bool]:
    """
    ...
    if existing:
        ...
        if is_edited:
            ...
            if len(data_delta) > 0:
                ...
                logger.info(
                    f"Updated submission {submission_id} (edited: {edit_reason}, {len(data_delta)} data changes)"
                )
            else:
                ...
                if not existing.has_edit_history:
                    ...
                    logger.debug(
                        f"Set has_edit_history for submission {submission_id} (deprecatedID present)"
                    )
                ...
                if metadata_changed:
                    logger.debug(
                        f"Updated metadata for submission {submission_id} (deprecatedID present but no data changes)"
                    )
                else:
                    logger.debug(
                        f"No changes for submission {submission_id} (deprecatedID present but already processed)"
                    )
            ...
            if existing.is_edited:
                ...
                logger.info(
                    f"Reset is_edited flag for submission {submission_id} (no deprecatedID found in current import)"
                )
