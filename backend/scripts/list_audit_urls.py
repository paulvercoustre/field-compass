...
def list_audit_urls_from_submissions():
    ...
    database_url = os.getenv('DATABASE_URL', 'postgresql://postgres:postgres@postgres:5432/field_compass')
    ...
    try:
        ...
        for submission in submissions:
            ...
            audit_related = {k: v for k, v in submission.submission_data.items() if 'audit' in k.lower()}
            if audit_related:
        ...
        print("      If submissions don't have active_interview_time, audit logs may not have been downloaded.")
...
def check_submission_audit_url(uuid: str):
    ...
    try:
        ...
        if has_active_time:
            active_time = submission.submission_data.get('active_interview_time')
            print(f"✅ Has processed audit log")
            print(f"   Active Interview Time: {active_time} minutes")
        else:
            print(f"❌ No processed audit log found")
            print(f"   (This could mean: audit URL was missing, download failed, or ETL hasn't run)")
        
        print()
        print("Note: To see the actual audit URL, check the ETL pipeline logs when processing this submission.")
