...
def find_survey_by_asset_id(kobo_asset_id: str):
    ...
    try:
        survey = db.query(SurveyConfig).filter(
            SurveyConfig.kobo_asset_id == kobo_asset_id
        ).first()
        ...
        if not survey:
            ...
            for s in all_surveys:
                print(f"  - {s.survey_name} (ID: {s.survey_id}, Asset: {s.kobo_asset_id or 'None'})")
            sys.exit(1)
        
        print(f"✅ Found survey:")
        print(f"  Survey ID: {survey.survey_id}")
        print(f"  Name: {survey.survey_name}")
        print(f"  Kobo Asset ID: {survey.kobo_asset_id}")
        print()
        print(f"To run ETL for this survey, use:")
