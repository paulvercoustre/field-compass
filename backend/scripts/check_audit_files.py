    ...
    for filename in sorted(csv_files):
        ...
        if metrics:
            ...
            print(
                ...
                else f"     Total Duration: {total_duration}"
            )
...
def check_specific_uuid(uuid: str):
    ...
    if os.path.exists(file_path):
        ...
        if metrics:
            ...
            print(
                f"   Total Duration: {metrics.get('total_duration'):.2f} minutes"
                if isinstance(metrics.get("total_duration"), int | float)
                else f"   Total Duration: {metrics.get('total_duration')}"
            )
            print(f"   Jump Count: {metrics.get('jump_count')}")
        else:
            print("   ⚠️  File exists but failed to process")
    else:
