...
Remember: "{dk_string}" and {dk_numeric} are valid "Don't Know" values."""
    ...
    def pretest_instrument(
        self,
        form: list[dict[str, Any]],
        profiles: list[dict[str, str]],
    ) -> list[dict[str, Any]]:
        """
        Walk a form as coverage profiles and report instrument defects.

        Schema only: ``form`` is question metadata (labels, types, choices,
        skip logic). No submission values, no open text from respondents.
        """
        if not self.is_available():
            raise ValueError("AI service is not available. Please configure OPENAI_API_KEY.")

        profile_text = "\n".join(
            f"- {item.get('id')}: {item.get('instruction')}" for item in profiles
        )
        system_prompt = """You are pre-testing a survey instrument, not predicting a population.

Walk the questionnaire as each coverage profile. Flag instrument defects. Do not invent
what a real household in a real country would answer. If a question is unanswerable from
the choices or skip logic, flag it rather than guessing.

Report only:
- unanswerable questions for that profile
- answers with no matching option
- double-barrelled items (two questions in one)
- ambiguous recall periods
- contradictions the form permits
- skip patterns that strand the respondent

Do not give generic questionnaire-design advice. Every finding must name a question_path
from the form. Use the profile id supplied, not a new name."""

        user_prompt = f"""COVERAGE PROFILES:
{profile_text}

FORM (schema only, no respondent data):
{json.dumps(form, ensure_ascii=False)}

Return findings for defects you actually observed."""
                ...
                "findings": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "check_id": {
                                "type": "string",
                                "description": "Short id, e.g. pretest_unanswerable",
                            },
                            "severity": {
                                "type": "string",
                                "enum": ["error", "warning", "info"],
                            },
                            "question_path": {"type": "string"},
                            "profile": {"type": "string"},
                            "message": {"type": "string"},
