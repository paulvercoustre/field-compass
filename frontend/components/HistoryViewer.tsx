...
const HistoryViewer: React.FC<HistoryViewerProps> = ({ history }) => {
  if (history.length === 0) {
    return <div className="p-4 text-center text-gray-500 bg-gray-50 dark:bg-gray-800 rounded-lg">No edit history found for this submission.</div>;
  }

  return (
    <div className="space-y-4 min-w-0">
      {history.map((entry) => (
        <div key={entry.history_id} className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg min-w-0">
          <div className="pb-3 mb-3 border-b border-gray-200 dark:border-gray-700 min-w-0">
            <p className="font-semibold text-gray-900 dark:text-white break-words">Change from {new Date(entry.timestamp).toLocaleString()}</p>
